...
```text
data/
  raw/          Minimal curated input workbooks used by the analysis scripts.
...
results/
  ...
  figures/                     Exploratory heterogeneity and Edgren drift figures.
...
- `data/raw/master_analysis_input.xlsx` - minimal master workbook used as the primary analysis input for manuscript calculations. It contains cleaned observation-level data, benchmark metadata, dataset metadata, tool-name harmonization, and Edgren truth-set annotations.
- `data/raw/edgren_rnafusion.xlsx` - curated RNAfusion output workbook used for the controlled Edgren/RNAfusion validation analysis.

These two workbooks are the only required input files for reproducing the analyses in this repository.
...
All values were manually extracted or curated from published benchmark articles and associated supplementary materials. The workbooks in `data/raw/` are the primary curated data sources. CSV files and figures in `results/` are derived outputs generated by the scripts.
